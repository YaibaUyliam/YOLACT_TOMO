...
from yolact_edge.data.kla_format import LENSEDetection,LENSEAnnotationTransform
...
parser.add_argument('--random_seed', default=51120, type=int,
...
port = 2 ** 15 + 2 ** 14 + hash(os.getuid()) % 2 ** 14 + 10
...
parser.add_argument('--keep_latest_interval', default=1000, type=int,
...
def train(rank, args):
    ...
    if rank == 0:
        save_folder = os.path.join(args.logs, "weight")
        print(save_folder)
    ...
    # set up logger
    log_folder = os.path.join(args.logs, "log")
    if not os.path.exists(log_folder):
            os.makedirs(log_folder)

    setup_logger(output=os.path.join(log_folder, cfg.name), distributed_rank=rank)
    logger = logging.getLogger("yolact.train")

    w = SummaryHelper(distributed_rank=rank, log_dir=os.path.join(log_folder, cfg.name))
    ...
    dataset = LENSEDetection(image_folder = cfg.dataset.train_images,
                                 info_folder= cfg.dataset.train_info,
                                 target_transform=LENSEAnnotationTransform(cfg=cfg),
                                 transform=SSDAugmentation(MEANS, cfg=cfg), cfg=cfg)

    if args.validation_epoch > 0:
        setup_eval()
        val_dataset = LENSEDetection(image_folder = cfg.dataset.train_images,
                                    info_folder= cfg.dataset.train_info,
                                    target_transform=LENSEAnnotationTransform(cfg=cfg),
                                    transform=BaseTransform(MEANS, cfg=cfg),cfg=cfg)
    # Set cuda device early to avoid duplicate model in master GPU
    if args.cuda:
        torch.cuda.set_device(rank)

    # Parallel wraps the underlying module, but when saving and loading we don't want that
    yolact_net = Yolact(cfg=cfg)
    net = yolact_net
    net.train()
    ...
    if args.resume is not None:
        ...
        # Recode auto download
        yolact_net.init_weights(backbone_path="/home/finn/yolact_edge/weights/" + cfg.backbone.path)
    ...
    scale_factor = args.num_gpus * args.batch_size // 8
    save_interval = len(dataset)/ args.batch_size // scale_factor
    epoch_size = len(dataset) // args.batch_size // args.num_gpus
    num_epochs = cfg.epochs
    max_iter = cfg.epochs * epoch_size
    ...
    def backward_and_log(prefix, net_outs, targets, masks, num_crowds, iteration, extra_loss=None):
        optimizer.zero_grad()
        ...
        for epoch in range(num_epochs):
            ...
            while True:
                ...
                if cfg.lr_warmup_until > 0 and iteration <= cfg.lr_warmup_until and cfg.lr_warmup_init < args.lr:
                    ...
                    set_lr(optimizer, args.lr * ((math.cos(math.pi * iteration / max_iter) + 1.) * .5))

                # Adjust the learning rate at the given iterations, but also if we resume from past that iteration
                #while cfg.lr_schedule == 'step' and step_index < len(cfg.lr_steps) and iteration >= cfg.lr_steps[step_index]:
                #    step_index += 1
                set_lr(optimizer, args.lr * math.exp(-args.gamma*iteration))
                ...
                losses = backward_and_log(run_name, net_outs, targets, masks, num_crowds, iteration)
                ...
                if cfg.dataset.is_video:
                    ...
                    if cfg.flow.base_backward:
                        losses = backward_and_log("compute", gt_net_outs, targets, masks, num_crowds, iteration)
                ...
                if iteration % 10 == 0:
                    ...
                    if torch.cuda.is_available():
                        ...
                        max_mem_mb = None
                    #losses = backward_and_log("compute", net_outs, targets, masks, num_crowds, iteration)
                    logger.info("""\
                ...
                if rank == 0 and iteration % save_interval == 0 and iteration != args.start_iter:
                    if args.keep_latest:
                    ...
                    if args.keep_latest and latest is not None:
                        if args.keep_latest_interval <= 0 or iteration % args.keep_latest_interval != save_interval:
            ...
            if args.validation_epoch > 0:
                if epoch % args.validation_epoch == 0 and epoch > 0:
                    if rank == 0:
                        mAP_values = compute_validation_map(yolact_net, val_dataset, cfg = cfg)

                        #w.add_scalar('mAP Box/IoU = 0.90', map_value)
                        for k in mAP_values.keys():
                            w.add_scalar('mAP Box/IoU = 0.{key}'.format(key=k), mAP_values[k])
                    misc.barrier()
...
def compute_validation_map(yolact_net, dataset,cfg=None):
    with torch.no_grad():
        yolact_net.eval()
        logger = logging.getLogger("yolact.eval")
        logger.info("Computing validation mAP (this may take a while)...")
        map_value = eval_script.evaluate(yolact_net, dataset, train_mode=True, train_cfg=cfg, cfg=cfg)
        yolact_net.train()
        return map_value
...
if __name__ == '__main__':
    print("===================")
    print(args.dataset, cfg.num_classes, cfg.name)
    print("===================")
