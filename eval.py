...
from yolact_edge.data import set_cfg, set_dataset
#from yolact_edge.model_pos.network import model_POS
...
def mask2polygon(mask):
        ...
        if results != []:
            for polygon in results[0]:

                list_x.append(int(polygon[0]))
                list_y.append(int(polygon[1]))

        return list_x,list_y, contours
...
def parse_args(argv=None):
    ...
    parser.add_argument('--collapse_nms', default=True, type=str2bool,
                        help='Whether to use a faster, but not entirely correct version of NMS.')
    ...
    parser.add_argument('--JSON_config', default=None,
                    help='The config object to use.')
    parser.set_defaults(no_bar=False, display=False, resume=False, output_json=False, output_web_json=False, shuffle=False,
...
# model_pos = model_POS()
# model_pos.load_weights('/home/finn/Tomo/best_weight/weights_300.h5')


def predict_pos(image_crop, x1, y1):
    img_resized = cv2.resize(image_crop, (128, 128))
    image = img_resized/ 255.0
    image = np.expand_dims(image, axis=0)
    _, position = model_pos.predict(image)
    pos=np.mean(position[0], 0)
    old_height, old_width = image_crop.shape[0], image_crop.shape[1]

    for i in range(len(pos)):    
        if i % 2 ==0:
            pos[i] = int((pos[i]*(old_width/128)+x1))
        else:
            pos[i] = int((pos[i]*(old_height/128)+y1))
...
def pos_convert(pos, img_old):

    old_height, old_width = img_old.shape[0], img_old.shape[1]

    for i in range(len(pos)):    
        if i % 2 ==0:
            pos[i] = int((pos[i]*(old_width/128)))
        else:
            pos[i] = int((pos[i]*(old_height/128)))

    return pos

def prep_display(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45,cfg=None,save_path=None, path=None, batch_idx=None):
    """
    ...
    if undo_transform:
        ...
        img_gpu = img / 255.0
        img_for_crop=(img_gpu*255).byte().cpu().numpy()
        h, w, _ = img.shape
    
    with timer.env('Postprocess'):
        t = postprocess(dets_out, w, h, visualize_lincomb = args.display_lincomb,
                                        crop_masks        = args.crop,
                                        score_threshold   = args.score_threshold,cfg=cfg, batch_idx=batch_idx)
    ...
    if num_dets_to_consider == 0:
        ...
        return (img_gpu * 255).byte().cpu().numpy(), None
    ...
    if args.display_masks and cfg.eval_mask_branch:
        ...
        #cv2.imwrite("/home/finn/yolact_edge/results/mask/{}.png".format(len(os.listdir("/home/finn/yolact_edge/results/mask")),masks_color_summand))
    ...
    if args.display_text or args.display_bboxes:
        for j in reversed(range(num_dets_to_consider)):
            ...
            # img_crop = img_for_crop[y1_new:y2_new,x1_new:x2_new]
            # pos = predict_pos(img_crop, x1_new, y1_new)
            # print(pos)
            ...
            if args.display_bboxes:
                ...
                #Visualize center
                # cv2.line(img_numpy, (int(pos[0]), int(pos[1])), (int(pos[2]), int(pos[3])), (0, 255, 0), 3)
                # cv2.line(img_numpy, (int(pos[0]), int(pos[1])), (int(pos[4]), int(pos[5])), (0, 0, 255), 3)

            binary_mask = masks[j, :, :, None].cpu().numpy() * 255
            binary_mask = binary_mask.astype(np.uint8)

            print(binary_mask.shape)
            #cv2.imwrite("/home/finn/Data_blister_5-7/mask/{}.png".format(str(len(os.listdir('/home/finn/Data_blister_5-7/mask')))),binary_mask[:,:,0,0])
            if args.output_json:
                kernelSize = 3
                ...
                list_x, list_y, contours = mask2polygon(cv2.morphologyEx(binary_mask[:,:,0,0], cv2.MORPH_OPEN, element, iterations=1))

                regions_data= {}
                regions_data["List_X"] = [float(x) for x in list_x]
                            #print()
                regions_data["List_Y"] = [float(y) for y in list_y]
                regions_data["Center"] = "0,0"
                regions_data["curTorelant"] = 0.0
                data_json["regions"][j] = regions_data
                classId.append(class_names[int(class_id)])
            if args.display_text:
                _class = cfg.dataset.class_names[classes[j]]
                text_str = '%s: %.2f' % (str(_class), score) if args.display_scores else _class
        ...
        if args.output_json:
                ...
                data_json["originalRegions"][j] = regions_data
                data_json["classId"] = classId
                data_json["listIsAutoLabel"] = [ False for x in range(num_dets_to_consider) ]
                data_json["pointAutoLabel"] = [ "0,0" for x in range(num_dets_to_consider) ]
                data_json["box"] = {
                                            "centerX": [],
                                            "centerY": [],
                                            "widthBox": [],
                ...
                data_json["filename"] = os.path.split(path)[-1][:-4] + '.bmp'
                data_json["width"] = img_numpy.shape[1]
                data_json["height"] = img_numpy.shape[0]
                with open(save_path[:-4] + '.bmp.json', 'w') as f:
                    json.dump(data_json, f)
    return img_numpy

def prep_benchmark(dets_out, h, w, cfg=None):
    with timer.env('Postprocess'):
        t = postprocess(dets_out, w, h, crop_masks=args.crop, score_threshold=args.score_threshold,cfg=cfg)
...
class Detections:
    ...
    def dump_web(self):
        ...
        output = {
            'info' : {
                'Config': {key: getattr(self.cfg, key) for key in config_outs},
            }
        }
...
def prep_metrics(ap_data, dets, img, gt, gt_masks, h, w, num_crowd, image_id, detections:Detections=None,cfg=None):
    ...
    with timer.env('Postprocess'):
        classes, scores, boxes, masks = postprocess(dets, w, h, crop_masks=args.crop, score_threshold=args.score_threshold, cfg=cfg)
...
class APDataObject:
    ...
    def get_ap(self) -> float:
        ...
        for datum in self.data_points:
            ...
            self.True_pos += num_true
            self.False_pos +=num_false
            self.False_neg += self.num_gt_positives - num_true
            self.True_neg += 0
...
list_t =[]
def evalimage(net:Yolact, paths:str, save_path:str=None, detections:Detections=None, image_id=None, cfg=None):
    frames=[]
    ...
    for path in paths:
        ...
        # img2[300:500,500:700] = img[300:500,500:700]
        # img = img2
        frames.append(img)
        
    frames= torch.from_numpy(np.array(frames)).cuda().float()
    #frame_pre = torch.cat(frame_pre, frame)
    batch = FastBaseTransform(cfg=cfg)(frames)
    ...
    for i in range(len(preds)):
        #print(i)
        img_numpy= prep_display(preds, frames[i], None, None, undo_transform=False,cfg=cfg, save_path=save_path[i], path=paths[i], batch_idx=i)
        #cv2.imwrite("/home/finn/yolact_edge/yolact_test/mask/{}.png".format(len(os.listdir("/home/finn/yolact_edge/yolact_test/mask"))), np.asarray(mask))
        #print("Predict time: ", (time.time() - start_t)*1000, " ms")
        ...
        if save_path is None:
            ...
            print(save_path[i])
            cv2.imwrite(save_path[i], img_numpy)
    

def evalimages(net:Yolact, input_folder:str, output_folder:str, detections:Detections=None,cfg=None, batch_size = 2):
    ...
    for i, p in enumerate(Path(input_folder).glob('*')):
        ...
        if len(paths)==batch_size:
            start = time.time()
            evalimage(net, paths, out_paths, detections=detections, image_id=str(i), cfg=cfg)
            end = time.time()
            paths = []
            out_paths=[]
    ...
    print('Done. Time:{} ms'.format(str(time_total/count*1000)))
...
def evalvideo(net:Yolact, path:str,cfg=None):
    ...
    def prep_frame(inp):
        with torch.no_grad():
            frame, preds = inp
            return prep_display(preds, frame, None, None, undo_transform=False, class_color=True,cfg=cfg)
...
def savevideo(net:Yolact, in_path:str, out_path:str,cfg=None):
    ...
    try:
        for i in range(num_frames):
            ...
            with timer.env('Video'):
                ...
                processed = prep_display(preds, frame, None, None, undo_transform=False, class_color=True,cfg=cfg)
...
def evaluate(net:Yolact, dataset, train_mode=False, train_cfg=None, cfg=None):
    net.detect.use_fast_nms = args.fast_nms
    net.detect.use_cross_class_nms= args.collapse_nms

    cfg.mask_proto_debug = args.mask_proto_debug

    detections = None
    # if args.output_json and (args.image or args.images):
    ...
    if args.image is not None:
        if ':' in args.image:
            ...
            evalimage(net, inp, out, detections=detections, image_id="0",cfg=cfg)
        else:
            evalimage(net, args.image, detections=detections, image_id="0",cfg=cfg)
        return

    elif args.images is not None:
        inp, out = args.images.split(':')
        evalimages(net, inp, out, detections=detections,cfg=cfg)
    ...
    try:
        for it, image_idx in enumerate(dataset_indices):
                ...
                if args.display:
                    img_numpy = prep_display(preds, img, h, w,cfg=cfg)
                elif args.benchmark:
                    prep_benchmark(preds, h, w,cfg=cfg)
                else:
                    prep_metrics(ap_data, preds, img, gt, gt_masks, h, w, num_crowd, dataset.ids[image_idx], detections,cfg=cfg)
        ...
        if not args.display and not args.benchmark:
            print()
            map_value = calc_map(ap_data,cfg=cfg)
        elif args.benchmark:
            print()
            print()
...
def calc_map(ap_data, cfg=None):
    logger = logging.getLogger("yolact.eval")
    logger.info('Calculating mAP...')
    aps = [{'box': [], 'mask': []} for _ in iou_thresholds]
    tfp=[{'box': {'TP': [], 'FP': [], 'TN': [], 'FN': []}, 'mask': {'TP': [], 'FP': [], 'TN': [], 'FN': []}} for _ in iou_thresholds]
        ...
        for iou_idx in range(len(iou_thresholds)):
            for iou_type in ('box', 'mask'):
                ...
                if not ap_obj.is_empty():
                    ...
                    True_pos, False_pos, True_neg, False_neg = ap_obj.get_TP_FP()

                    tfp[iou_idx][iou_type]['TP'].append(True_pos)
                    tfp[iou_idx][iou_type]['FP'].append(False_pos)
                    tfp[iou_idx][iou_type]['TN'].append(True_neg)
                    tfp[iou_idx][iou_type]['FN'].append(False_neg)
...
def print_maps(all_maps):
    ...
    map_value = OrderedDict()
    for x in all_maps['box'].keys():
        if x in (80,90):
            map_value[x]=all_maps['box'][x]
            print(map_value)
    return map_value
...
if __name__ == '__main__':
    ...
    cfg = LoadJSON_config(args.JSON_config)._load_config()
    ...
    with torch.no_grad():
        ...
        net = Yolact(training=False,cfg=cfg)
        ...
        #convert_to_tensorrt(net, cfg, args, transform=BaseTransform(cfg=cfg))

        # if args.cuda:
        #     net = net.cuda()

        # start = time.time()
        evaluate(net, dataset,cfg=cfg)
        # end = time.time()
        # print(end - start)
