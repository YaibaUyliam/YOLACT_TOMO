...
from yolact_edge.data import MEANS, STD
from .contrastStretching import contrast_stretching
from .custom_augmentations import augment_data
...
class Resize(object):
    ...
    def __init__(self, resize_gt=True, cfg=None):
...
class BaseTransform(object):
    ...
    def __init__(self, mean=MEANS, std=STD, cfg=None):
        self.augment = Compose([
            ConvertFromInts(),
            Resize(resize_gt=False, cfg=cfg),
...
class FastBaseTransform(torch.nn.Module):
    ...
    def forward(self, img):
        ...
        if type(self.cfg.max_size) == tuple:
            img = F.interpolate(img, self.cfg.max_size[::-1], mode='bilinear', align_corners=False)
        else:
            img = F.interpolate(img, (self.cfg.max_size, self.cfg.max_size), mode='bilinear', align_corners=False)
...
# Phase 1.0 ****** Another augmentation ways *********************************************
...
def another_augmentation(image):
    masks_list = [np.zeros((image.shape[:2]))]
    transformed = get_training_augmentation()(image=image, masks=masks_list)
    image, proc_masks = transformed['image'], transformed['masks']
