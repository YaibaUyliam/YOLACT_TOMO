...
from yolact_edge.data.config import mask_type
...
class PredictionModule(nn.Module):
    ...
    def __init__(self, in_channels, out_channels=1024, aspect_ratios=[[1]], scales=[1], parent=None, index=0, cfg=None):
        super().__init__()
                    ...
                    for scale, ars in zip(self.scales, self.aspect_ratios):
                        for ar in ars:
                            ...
                            if self.cfg.backbone.use_pixel_scales:
                                if type(self.cfg.max_size) == tuple:
                                    width, height = self.cfg.max_size
                                    w = scale * ar / width
                                    h = scale / ar / height
                                else:
...
class PredictionModuleTRT(PredictionModule):
    ...
    def forward(self, x):
        ...
        if self.cfg.use_instance_coeff:
            inst = self.inst_layer(x).permute(0, 2, 3, 1).contiguous().view(x.size(0), -1, self.cfg.num_instance_coeffs)
            raise NotImplementedError
...
class FlowNetMiniTRTWrapper(nn.Module):
    def __init__(self, flow_net, cfg=None):
        super().__init__()
...
class PredictionModuleTRTWrapper(nn.Module):
    def __init__(self, pred_layer, cfg=None):
        super().__init__()
        self.pred_layer = PredictionModuleTRT(*pred_layer.params[:-2], None, pred_layer.params[-1],cfg=cfg)
...
class FlowNetMiniPredLayer(nn.Module):
    def __init__(self, in_features,cfg=None):
        super().__init__()
...
class FlowNetMiniPreConvs(ScriptModuleWrapper):
    def __init__(self, reduce_channels, cfg=None):
        super().__init__()
...
def build_flow_convs(encode_layers, in_features, out_features, stride=1, groups=1, cfg=None):
    conv = []
...
class FlowNetMini(ScriptModuleWrapper):
    ...
    def __init__(self, in_features,cfg=None):
        ...
        self.conv1 = build_flow_convs(cfg.flow.encode_layers[0], in_features, cfg.flow.encode_channels, groups=cfg.flow.num_groups,cfg=cfg)
        self.conv2 = build_flow_convs(cfg.flow.encode_layers[1], cfg.flow.encode_channels, cfg.flow.encode_channels * 2, stride=2,cfg=cfg)
        self.conv3 = build_flow_convs(cfg.flow.encode_layers[2], cfg.flow.encode_channels * 2, cfg.flow.encode_channels * 4, stride=2,cfg=cfg)
...
class SPA(ScriptModuleWrapper):
    ...
    def __init__(self, num_layers, cfg=None):
        super().__init__()
        self.interpolation_mode = cfg.fpn.interpolation_mode
...
class Yolact(nn.Module):
    ...
    def __init__(self, training=True, cfg=None):
        ...
        if cfg.fpn is not None:
            ...
            if cfg.flow is not None:
                self.fpn_phase_1 = FPN_phase_1([src_channels[i] for i in self.selected_layers], cfg=cfg)
                self.fpn_phase_2 = FPN_phase_2([src_channels[i] for i in self.selected_layers], cfg=cfg)
                if cfg.flow.use_spa:
                    self.spa = SPA(len(self.selected_layers),cfg=cfg)
                if cfg.flow.warp_mode == 'flow':
                    if cfg.flow.model == 'mini':
                        ...
                        self.flow_net_pre_convs = FlowNetMiniPreConvs(cfg.flow.reduce_channels, cfg=cfg)
                        ...
                        if not training and (cfg.torch2trt_flow_net or cfg.torch2trt_flow_net_int8):
                            self.flow_net = FlowNetMiniTRT(lateral_channels * 2, cfg=cfg)
                        else:
                            self.flow_net = FlowNetMini(lateral_channels * 2, cfg=cfg)
                    else:
                        raise NotImplementedError
        ...
        self.detect = Detect(cfg.num_classes, bkg_label=0, top_k=200, conf_thresh=0.05, nms_thresh=0.5, cfg=cfg)
    ...
    def load_weights(self, path, args=None):
        ...
        if not self.training:
            ...
            if self.cfg.torch2trt_flow_net or self.cfg.torch2trt_flow_net_int8:
    ...
    def train(self, mode=True):
        ...
        if self.cfg.flow is not None and self.cfg.flow.fine_tune_layers is not None:
    ...
    def fine_tune_layers(self):
        fine_tune_layers = self.cfg.flow.fine_tune_layers
    ...
    def extra_loss(self, net_outs, gt_net_outs):
        ...
        if self.cfg.flow.feature_matching_loss is not None:
            def t(fea, layer_idx):
            ...
            assert self.cfg.flow.fm_loss_loc in ("L", "P", "L+P")
            loss_W = 0
            ...
            loss_W /= len(pairs)
            losses['W'] = loss_W * self.cfg.flow.fm_loss_alpha
        return losses

    def forward_flow(self, extras):
        imgs_1, imgs_2 = extras

        if self.cfg.flow.model == 'mini':
    ...
    def create_partial_backbone(self):
        if self.cfg.flow.warp_mode == 'none':
            return

        logger = logging.getLogger("yolact.model.load")
        logger.debug("Creating partial backbone...")

        backbone = construct_backbone(self.cfg.backbone)
    ...
    def _get_trt_cache_path(self, module_name, int8_mode=False, batch_size=1):
        return "{}.{}{}{}.trt".format(self.model_path, module_name, ".int8_{}".format(self.cfg.torch2trt_max_calibration_images) if int8_mode else "", "_bs_{}".format(batch_size))
    ...
    def to_tensorrt_backbone(self, int8_mode=False, calibration_dataset=None, batch_size=1):
        ...
        x = torch.ones((1, 3, self.cfg.max_size, self.cfg.max_size)).cuda()
    ...
    def to_tensorrt_fpn(self, int8_mode=False, calibration_dataset=None, batch_size=1):
        ...
        if self.cfg.backbone.name == "ResNet50" or self.cfg.backbone.name == "ResNet101":
            x = [
                torch.randn(1, 512, 69, 69).cuda(),
                torch.randn(1, 1024, 35, 35).cuda(),
                torch.randn(1, 2048, 18, 18).cuda(),
                ]
        elif self.cfg.backbone.name == "MobileNetV2":
            x = [
            ...
            raise ValueError("Backbone: {} is not currently supported with TensorRT.".format(self.cfg.backbone.name))
    ...
    def to_tensorrt_prediction_head(self, int8_mode=False, calibration_dataset=None, batch_size=1):
        ...
        for idx, pred_layer in enumerate(self.prediction_layers):
            pred_layer = PredictionModuleTRTWrapper(pred_layer,cfg=self.cfg)
    ...
    def to_tensorrt_flow_net(self, int8_mode=False, calibration_dataset=None, batch_size=1):
        ...
        lateral_channels = self.cfg.fpn.num_features
        if len(self.cfg.flow.reduce_channels) > 0:
            lateral_channels = self.cfg.flow.reduce_channels[-1]
    ...
    def forward(self, x, extras=None):
        ...
        if self.cfg.flow.train_flow:
            return self.forward_flow(extras)

        outs_wrapper = {}

        with timer.env('backbone'):
            if self.cfg.flow is None or extras is None or extras["backbone"] == "full":
        ...
        if self.cfg.flow is not None:
            with timer.env('fpn'):
                ...
                if extras["backbone"] == "full":
                    ...
                    if self.cfg.flow.warp_mode != 'take':
                    ...
                    if self.cfg.flow.warp_mode == "flow":
                        with timer.env('flow'):
                            ...
                            for (flow, scale_factor, scale_bias), feat in zip(flows, outs_phase_1):
                                ...
                                if self.cfg.flow.use_scale_factor:
                                    pred_feat *= scale_factor
                                if self.cfg.flow.use_scale_bias:
                    ...
                    if self.cfg.flow.warp_layers == 'P4P5':
                        with timer.env('p3'):
                            ...
                            src_fpn = F.interpolate(src_fpn, size=(h, w), mode=self.cfg.fpn.interpolation_mode, align_corners=False)
                            p3 = src_fpn + lateral

                            outs_phase_1 = [p3] + outs_phase_1

                    if self.cfg.flow.use_spa:
                        with timer.env('spa'):
        ...
        with timer.env('pred_heads'):
            ...
            for idx, pred_layer in zip(self.selected_layers, self.prediction_layers):
                ...
                if self.training or not (self.cfg.torch2trt_prediction_module or self.cfg.torch2trt_prediction_module_int8):
