...
def mask2poly_single(binary_mask):
    ...
    list_x, list_y = TuplePoly2Poly(poly)
    return  list_x, list_y
...
# model_pos = model_POS()
# model_pos.load_weights('/home/finn/Tomo/weight_3/weights_300.h5')
...
def prep_display(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45,cfg=None):
    """
    ...
    with timer.env('Postprocess'):
        t = postprocess(dets_out, w, h, visualize_lincomb = args.display_lincomb,
                                        ...
                                        score_threshold   = args.score_threshold,cfg=cfg)
    ...
    ## Show some classes  
    # class_attention = ["tray_hole", "printer_button", "push_rod", "cuvette_hole", "printer_hole", "swing_bar", "tray"]
    # classes_cp = copy.deepcopy(classes)
    # id_delete = 0

    # for value in classes_cp:
    #     if cfg.dataset.class_names[value] not in class_attention:
    #         classes = np.delete(classes, id_delete)
    #         scores = np.delete(scores, id_delete, 0)
    #         boxes = np.delete(boxes, id_delete, 0)
    #         masks = torch.cat((masks[:id_delete,:, :], masks[id_delete+1:,:,:]), axis = 0)
    ...
    if args.display_text or args.display_bboxes:
        for j in reversed(range(num_dets_to_consider)):
            ...
            if args.display_bboxes:
                ...
                #cv2.circle(img_numpy, (int(pos[0]),int(pos[1])), 1, (0,0,255), 2) #center
                #cv2.circle(img_numpy, (int(pos[2]),int(pos[3])), 1, (255,0,0), 2) 
                #cv2.circle(img_numpy, (int(pos[4]),int(pos[5])), 1, (0,255,0), 2)
                # cv2.arrowedLine(img_numpy, (int(pos[0]), int(pos[1])), (int(pos[2]),int(pos[3])),(0,0,255), 3, 8, 0, 0.1)
                # cv2.arrowedLine(img_numpy, (int(pos[0]), int(pos[1])), (int(pos[4]),int(pos[5])),(0,255,0), 3, 8, 0, 0.1)

                #img_crop = img_for_crop[y1:y2, x1:x2]
                #cv2.imwrite("/home/finn/yolact_edge/images/crop/{}.png".format(len(os.listdir("/home/finn/yolact_edge/images/crop"))), img_crop)
            binary_mask = masks[j, :, :, None].cpu().numpy() * 255
            binary_mask = binary_mask.astype(np.uint8)
            #print(binary_mask.shape)
            
            # #cv2.imwrite("/home/finn/yolact_edge/results/mask/{}.png".format(len(os.listdir("/home/finn/yolact_edge/results/mask"))),binary_mask[:,:,0,0])
            # list_x, list_y = mask2poly_single(binary_mask[:,:,0,0])

            # print("List_x: ", list_x)
            # print("List_y: ", list_y)
            # print(x1," ", y1," ", x2," ", y2)
            # centerx,centery = (x1+x2)//2, (y1+y2)//2
            # is_cap = is_bottleCap(img_numpy[y1:y2,x1:x2])
    ...
    return img_numpy,binary_mask[:,:,0,0]
...
list_t =[]
def evalimage(net:Yolact, path:str, save_path:str=None, detections:Detections=None, image_id=None, cfg=None):
    ...
    batch = FastBaseTransform(cfg=cfg)(frame.unsqueeze(0))
    ...
    img_numpy,mask = prep_display(preds, frame, None, None, undo_transform=False,cfg=cfg)

    list_t.append((time.time() - start_t)*1000)

    print(sum(list_t)/len(list_t))
    #print("Predict time: ", (time.time() - start_t)*1000, " ms")

    try:
        cv2.imwrite("test/" + "mask"+ os.path.basename(path), np.asarray(mask))
    except:
        pass
    ...
    if args.output_coco_json:
        with timer.env('Postprocess'):
            ...
            classes, scores, boxes, masks = \
                postprocess(preds, w, h, crop_masks=args.crop, score_threshold=args.score_threshold,cfg=cfg)
...
def evalimages(net:Yolact, input_folder:str, output_folder:str, detections:Detections=None,cfg=None):
    ...
    for i, p in enumerate(Path(input_folder).glob('*')):
        ...
        evalimage(net, path, out_path, detections=detections, image_id=str(i), cfg=cfg)
        print(path + ' -> ' + out_path )
...
if __name__ == '__main__':
    ...
    model.load_weights(pretrained, args=args)
    model.eval()
    model = model.cuda()
